<script context="module">
	...
	export async function load({ page, fetch, session, context }) {
		...
		try {
            console.log('page.query dbInfo', page.query.get('dbInfo'))
            const dbInfo = JSON.parse(page.query.get('dbInfo'))
            console.log('dbInfo', dbInfo)
			const meeting = await getMeeting(dbInfo)
