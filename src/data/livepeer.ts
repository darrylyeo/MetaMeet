...
export const fetchStreams = async (
	...
	if (options.only === 'streams')
		params.append('streamsonly', '1')
	if (options.only === 'sessions')
		params.append('sessionsonly', '1')
	if (options.record)
		params.append('record', '1')
	if (options.isActive)
		params.append('filters', '[{id:"isActive",value:true}]')
		
	const res = await fetch(`https://livepeer.com/api/stream?${params}`, {
		method: 'GET',
		headers: {'Authorization': `Bearer ${apiKey}`},
	})
